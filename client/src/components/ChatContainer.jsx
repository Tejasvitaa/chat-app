...
const ChatContainer = () => {
  ...
  const handleSendMessage=async(e)=>{
    if(e && e.preventDefault) // check if e is defined and has preventDefault method
    e.preventDefault();
  ...
  return selectedUser? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
        ...
        {messages.filter(Boolean).map((msg, index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId!== authUser._id && 'flex-row-reverse'}`}>
            ...
            <div className="text-center text-xs">
              <img src={msg.senderId===authUser._id?assets.authUser?.profilePic||assets.avatar_icon:selectedUser?.profilePic||assets.avatar_icon} alt="" className="w-7 rounded-full"/>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-[#282142] p-3 flex items-center gap-3">
